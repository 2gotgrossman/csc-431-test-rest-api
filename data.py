...
LAT_LONGS = [{'latitude': 0.0, 'longitude': 0.0} for i in range(1)]
...
def HAZARD_DATA_TEMPLATE(num_images):
    ...
    'images_by_satellite': {
        ...
        'satellite_id2': {image_type: IMAGE_TYPE_DATA_TEMPLATE(num_images) for image_type in SUPPORTED_IMAGE_TYPES},
    }
}
...
def get_volcano_hazard_info_by_id(hazard_id, image_types, satellites, max_num_images):
    ...
    if len(satellites) != 0:
        for satellite in HAZARD_DATA_TEMPLATE['images_by_satellite']:
